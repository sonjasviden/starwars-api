...
import { ResultVehicles } from "../types"
...
const VehiclesPage = () => {
    const [data, setData] = useState<ResultVehicles | null>(null)
    ...
    const getVehicles = async () => {
        ...
        try {
            const res = await swapi.getVehicles(page)
            setData(res)
            console.log(res)
        } catch (err: any) {
    ...
    const searchStarWars = async (searchQuery: string, searchPage = 1) => {
        ...
        try {
            const res = await swapi.getSearchVehicles(searchQuery, searchPage)
            setData(res)
            console.log(res)
        } catch (err: any) {
            setError(err.message)
                            ...
                            <div className="cards-list" key={data.id}>
                                <div className="card">
                                    ...
                                    <div className="readMore-div">
                                        <Button variant="dark">
                                            <Link to={`/vehicles/${data.id}`}>Read more</Link>
                                        </Button>
                                    </div>
                                </div>
                            </div>
