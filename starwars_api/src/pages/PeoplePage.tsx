...
import { ResultPeople } from "../types"
...
const PeoplesPage = () => {
    const [data, setData] = useState<ResultPeople | null>(null)
    ...
    const getPeople = async () => {
        ...
        try {
            const res = await swapi.getPeople(page)
            setData(res)
            console.log(res)
        } catch (err: any) {
    ...
    const searchStarWars = async (searchQuery: string, searchPage = 1) => {
        ...
        try {
            const res = await swapi.getSearchPeople(searchQuery, searchPage)
            setData(res)
            console.log(res)
        } catch (err: any) {
            setError(err.message)
                            ...
                            <div className="cards-list" key={data.id}>
                                <div className="card">
                                    ...
                                    <div className="readMore-div">
                                        <Button variant="dark">
                                            <Link to={`/people/${data.id}`}>Read more</Link>
                                        </Button>
                                    </div>
                                </div>
                            </div>
