...
import { ResultStarships } from "../types"
...
const StarshipsPage = () => {
    const [data, setData] = useState<ResultStarships | null>(null)
    ...
    const getStarships = async () => {
        ...
        try {
            const res = await swapi.getStarships(page)
            setData(res)
            console.log(res)
        } catch (err: any) {
    ...
    const searchStarWars = async (searchQuery: string, searchPage = 1) => {
        ...
        try {
            const res = await swapi.getSearchStarships(searchQuery, searchPage)
            setData(res)
            console.log(res)
        } catch (err: any) {
            setError(err.message)
    ...
    return (
        <main>
            <h2 className="heading">Starships</h2>
                            ...
                            <div className="cards-list" key={data.id}>
                                <div className="card">
                                    ...
                                    <div className="readMore-div">
                                        <Button variant="dark">
                                            <Link to={`/starships/${data.id}`}>Read more</Link>
                                        </Button>
                                    </div>
                                </div>
                            </div>
